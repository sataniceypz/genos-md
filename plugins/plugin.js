...
Function({
	...
	if (!match) return await message.reply('*which plugin? enter plugin name !*\n_Example :_\n.remove mforward\n.remove all');
	try {
		if (match.toLowerCase() === 'all') {
			...
			return await message.reply('_ successfully deleted!_\n*Reboot the BOT*');
		}
		...
		await message.reply('_ successfully deleted!_\n*Reboot the BOT*');
	} catch (error) {
		console.error(error);
